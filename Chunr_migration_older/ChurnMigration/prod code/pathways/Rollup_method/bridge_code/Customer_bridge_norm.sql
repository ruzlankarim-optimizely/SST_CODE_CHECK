...
SELECT *
FROM ufdm_archive.sst_customer_bridge_lcoked_17122024_0238 scb;
...
select
    ...
    and (counting_migration <> count_movements )
    and abs(total_migration_amount_ccfx) > abs(customer_arr_change_ccfx)
    then Case when (migration_rolled_up_amount_lcu/ total_migration_amount_lcu ) * 2 = 1 then round((migration_rolled_up_amount_lcu / total_migration_amount_lcu) * (total_migration_amount_ccfx - customer_arr_change_lcu))
