...
SELECT *
FROM ufdm_archive.sst_pb_product_solution_cloud_license_lcoked_10122024_1354;
...
drop table if exists sandbox.sandbox.churn_migration_limitation_ps_cloud;
create table sandbox.sandbox.churn_migration_limitation_ps_cloud as
    with base as (
        Select
            mcid ,
            evaluation_period ,
...
create table sandbox.solution_churn_migration_default_cases_cloud as
    ...
    from sandbox.sst_product_solution_bridge_rollup_cm_cloud as a
    join sandbox.sandbox.churn_migration_limitation_ps_cloud as b
    on a.evaluation_period = b.evaluation_period
    and a.mcid = b.mcid
    and a.currency_code = b.currency_code
...
Drop table if exists sandbox.sandbox.solution_churn_migration_split_cases_cloud ;
create table sandbox.sandbox.solution_churn_migration_split_cases_cloud as
    Select
        a.*,
        b.product_solutions ,
        b.pathways as migration_pathways ,
...
Delete from sandbox.sst_product_solution_bridge_rollup_cm_cloud as a
Using sandbox.sandbox.solution_churn_migration_split_cases_cloud as b
where a.mcid = b.mcid
and a.evaluation_period = b.evaluation_period
and a.currency_code  = b.currency_code
and concat(a.prior_product_solution, '=',a.current_product_solution) = b.product_solutions
...
FROM sandbox.sandbox.solution_churn_migration_split_cases_cloud AS b
WHERE mcid = b.mcid
  AND evaluation_period = b.evaluation_period
  AND currency_code = b.currency_code
  AND concat(prior_product_solution, '=',current_product_solution) = b.product_solutions
