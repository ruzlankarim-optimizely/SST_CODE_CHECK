CREATE OR REPLACE FUNCTION ryzlan.sp_populate_adaptive_exports_churn_mig_renewal_type(snapshot_date_from date, snapshot_date_to date) RETURNS void LANGUAGE plpgsql AS $function$ BEGIN --####################################################
...
create temp table temp_adaptive_ending_ARR as with temp_subentity as (
  ...
  from (
      ...
      from ufdm_archive.sst_adhoc_lcoked_17062024_0940
...
update ryzlan.SST_adaptive_ending_ARR a
set lapsed_flag = 'Y'
from (
    select mcid,
      sb.current_period as snapshot_date
    from sandbox.sst_customer_bridge_cm sa
      join ufdm_grey.periods sb on sa.evaluation_period = sb.evaluation_period
    where customer_bridge = 'Lapsed Renewal'
    union
    select mcid,
      sb.current_period as snapshot_date
    from sandbox.sst_customer_bridge_cm sa
...
where a.master_customer_id = b.mcid
  and a.snapshot_date = b.snapshot_date;
--####################################################
--Customer metadata
--####################################################
...
from sandbox.sst_product_group_bridge a
...
case
  ...
  product_arr_change_ccfx,
  pathways,
  null as renewal_type --select distinct evaluation_period
from sandbox.sst_product_solution_bridge a
  join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
where 1 = 1 --and a.product_bridge not in ('Flat','N/A')
  and product_arr_change_ccfx <> 0 --and mcid is not null and mcid <> ''
  and b.current_period between snapshot_date_from and snapshot_date_to
...
case
  ...
  pathways,
  null as renewal_type --,a.evaluation_period
  --select *
from sandbox.sst_customer_bridge_cm a
  join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
...
from temp_adaptive_ending_ARR
where coalesce(baseline_arr_local_currency, 0) > 0;
update ryzlan.SST_adaptive_ending_ARR_split a
set lapsed_flag = 'Y'
from (
    select mcid,
      sb.current_period as snapshot_date
...
--   pathways,
--   renewal_type
-- from sandbox_pd.sst_product_group_bridge_cloud_churn_mig a
--   join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
-- where 1 = 1 --and a.product_bridge not in ('Flat','N/A')
--   and product_arr_change_ccfx <> 0 --and mcid is not null and mcid <> ''
--   and b.current_period between snapshot_date_from and snapshot_date_to
...
--   pathways,
--   renewal_type
-- from sandbox_pd.sst_pb_product_solution_cloud_license_churn_mig a
--   join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
-- where 1 = 1 --and a.product_bridge not in ('Flat','N/A')
--   and product_arr_change_ccfx <> 0 --and mcid is not null and mcid <> ''
--   and b.current_period between snapshot_date_from and snapshot_date_to
...
--   a.customer_arr_change_ccfx,
--   pathways,
--   renewal_type --,a.evaluation_period
--   --select count(*)
-- from sandbox_pd.sst_customer_bridge_core_churn_mig a
--   join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
-- where 1 = 1 --and a.customer_bridge not in ('Flat','N/A')
--   and customer_arr_change_ccfx <> 0
--   and b.current_period between snapshot_date_from and snapshot_date_to
...
--     and p.current_period = b.snapshot_date
--   where 1 = 1
--     and coalesce(current_product_solution, prior_product_solution) in ('Digital Optimization', 'Monetize', 'Orchestrate')
--   group by b.master_customer_id,
--     b.snapshot_date,
...
--   renewal_type --,a.evaluation_period
--   --select count(*)
-- from sandbox_pd.sst_customer_bridge_cloud_churn_mig a
--   join ufdm_grey.periods b on a.evaluation_period = b.evaluation_period
-- where 1 = 1 --and a.customer_bridge not in ('Flat','N/A')
--   and customer_arr_change_ccfx <> 0
--   and b.current_period between snapshot_date_from and snapshot_date_to
...
--     and p.current_period = b.snapshot_date
--   where 1 = 1
--     and coalesce(current_product_solution, prior_product_solution) in (
...
-- where a.master_customer_id = b.master_customer_id
--   and a.snapshot_date = b.snapshot_date;
--     alter table sandbox_pd.sst_adaptive_ending_arr add column Id serial primary key;
--     alter table sandbox_pd.sst_adaptive_customer_metadata add column Id int primary key;
--     alter table sandbox_pd.sst_adaptive_customer_bridge_movements add column Id int primary key;
--     alter table sandbox_pd.sst_adaptive_product_bridge_pg_movements add column Id int primary key;
--     alter table sandbox_pd.sst_adaptive_product_bridge_ps_movements add column Id int primary key;
--     alter table sandbox_pd.sst_adaptive_ending_arr_split add column Id int primary key;
