...
SELECT *
FROM ufdm_archive.sst_customer_bridge_lcoked_25062024_2329 scb;
--#############################################
--CHURN MIGRATION
--#############################################
...
CREATE temp table churn_migration_classifiers_pg as (
  WITH initial_table_4 as (
    ...
    WHERE rt.customer_bridge IN (
        ...
        'Price Uplift',
        'Downgrade'
      ) --      AND it3.mcid =   'cd645167-5fd7-e211-9350-0050568d002c'  
      --    AND it3.evaluation_period  =  '2020M01'
...
,
  ...
  ) --    SELECT * FROM double_classification_marker WHERE  "PG Migration: Classification" IS NOT NULL 
