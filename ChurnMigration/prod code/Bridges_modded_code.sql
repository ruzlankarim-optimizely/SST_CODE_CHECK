...
DROP TABLE IF EXISTS sandbox.PG_modded;
CREATE TABLE sandbox.PG_modded AS SELECT * FROM ufdm.sst_product_bridge_product_group ;

ALTER TABLE sandbox.PG_modded DROP COLUMN id;
...
FROM sandbox.churn_migration_main 
WHERE mcid NOT IN ('-');
...
DELETE FROM  sandbox.PG_modded AS a 
...
INSERT INTO sandbox.PG_modded AS a (
   evaluation_period ,
    prior_period ,
    current_period ,
...
DROP TABLE IF EXISTS sandbox.PS_modded;
CREATE TABLE sandbox.PS_modded AS
SELECT *
FROM ufdm.sst_product_bridge_product_solution;
ALTER TABLE sandbox.PS_modded DROP COLUMN id;
...
DELETE FROM sandbox.PS_modded AS a USING sandbox.PS_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND a.prior_product_solution = b.prior_product_solution
  AND a.current_product_solution = b.current_product_solution
  AND a.product_bridge = b.product_bridge;
INSERT INTO sandbox.PS_modded AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
WHERE mcid = b.mcid
  ...
  AND product_bridge = b.product_bridge;
DELETE FROM sandbox.PS_modded AS a USING sandbox.PS_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND a.prior_product_solution = b.prior_product_solution
...
WHERE mcid = b.mcid
  ...
  DROP TABLE IF EXISTS sandbox.CB_modded;
CREATE TABLE sandbox.CB_modded AS
SELECT *
FROM ufdm.sst_customer_bridge scb;
ALTER TABLE sandbox.CB_modded DROP COLUMN id;
...
DELETE FROM sandbox.CB_modded AS a USING sandbox.CB_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
INSERT INTO sandbox.CB_modded AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
...
DELETE FROM sandbox.CB_modded AS a USING sandbox.CB_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
