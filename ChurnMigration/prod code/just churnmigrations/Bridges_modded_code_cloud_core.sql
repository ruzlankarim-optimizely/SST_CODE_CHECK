...
-- PRODUCT GROUP BRIDGE MODDED cloud
DROP TABLE IF EXISTS sandbox.PG_modded_cloud;
CREATE TABLE sandbox.PG_modded_cloud AS SELECT * FROM ufdm.sst_product_bridge_product_group_cloud_license ;

ALTER TABLE sandbox.PG_modded_cloud DROP COLUMN id;
...
FROM sandbox.churn_migration_main_cloud 
WHERE mcid NOT IN ('-');

DROP TABLE IF EXISTS sandbox.PG_migration_default ;
CREATE TABLE sandbox.PG_migration_default AS 
SELECT a.* ,"PG Migration: Rolled Up Amount", "PG Leftover: Rolled Up Amount", "PG Migration: Rolled Up Amount LCU", "PG Leftover: Rolled Up Amount LCU" ,pg_bridge,"PG Migration: Classification"
FROM sandbox.PG_modded_cloud AS a
JOIN sandbox.churn_migration_test_pg AS b 
ON a.mcid = b.mcid  
...
SELECT a.*, "PG Migration: Rolled Up Amount", "PG Leftover: Rolled Up Amount", "PG Migration: Rolled Up Amount LCU", "PG Leftover: Rolled Up Amount LCU" ,"PG Migration: Classification","PG Leftover: Classification"
FROM sandbox.PG_modded_cloud AS a 
JOIN sandbox.churn_migration_test_pg AS b 
ON a.mcid = b.mcid  
AND a.evaluation_period = b.evaluation_period 
...
AND a.prior_product_group  = b.prior_product_group 
AND a.product_bridge = b.pg_bridge  
--AND round(a.product_arr_change_ccfx)  = round(b.pg_arr_change) 
...
DELETE FROM  sandbox.PG_modded_cloud AS a 
...
INSERT INTO sandbox.PG_modded_cloud AS a (
   evaluation_period ,
    prior_period ,
    current_period ,
...
SELECT 
    ...
    prior_product_group ,
    currency_code ,
    prior_period_product_arr_usd_ccfx  * abs("PG Migration: Rolled Up Amount" /product_arr_change_ccfx),
    current_period_product_arr_usd_ccfx * abs("PG Migration: Rolled Up Amount" /product_arr_change_ccfx),
--  product_arr_change_ccfx ,
    "PG Migration: Rolled Up Amount" ,
    prior_period_product_arr_lcu  * abs("PG Migration: Rolled Up Amount LCU" /product_arr_change_lcu) ,
    current_period_product_arr_lcu   * abs("PG Migration: Rolled Up Amount LCU" /product_arr_change_lcu),
    "PG Migration: Rolled Up Amount LCU",
--    product_bridge ,
    COALESCE("PG Migration: Classification",product_bridge) , 
    winback_period_days ,
...
SELECT evaluation_period ,
    ...
    prior_product_group ,
    currency_code ,
    prior_period_product_arr_usd_ccfx * abs("PG Leftover: Rolled Up Amount" / product_arr_change_ccfx),
    current_period_product_arr_usd_ccfx * abs("PG Leftover: Rolled Up Amount" / product_arr_change_ccfx),
--  product_arr_change_ccfx ,
-- change this to default once and then to migrated value
    "PG Leftover: Rolled Up Amount" ,
    prior_period_product_arr_lcu * abs("PG Leftover: Rolled Up Amount LCU" / product_arr_change_lcu),
    current_period_product_arr_lcu  * abs("PG Leftover: Rolled Up Amount LCU" / product_arr_change_lcu),
    "PG Leftover: Rolled Up Amount LCU",
--    default_value_lcu ,
--    product_bridge ,
...
----- PRODUCT SOLUTION MODDED Cloud


DROP TABLE IF EXISTS sandbox.PS_modded_cloud;
CREATE TABLE sandbox.PS_modded_cloud AS
SELECT *
FROM ufdm.sst_product_bridge_product_solution_cloud_license;
ALTER TABLE sandbox.PS_modded_cloud DROP COLUMN id;
...
SELECT a.*,
  ...
  ps_bridge,
  "PS Migration: Classification"
FROM sandbox.PS_modded_cloud AS a
...
DELETE FROM sandbox.PS_modded_cloud AS a USING sandbox.PS_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND a.prior_product_solution = b.prior_product_solution
  AND a.current_product_solution = b.current_product_solution
  AND a.product_bridge = b.product_bridge;
INSERT INTO sandbox.PS_modded_cloud AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
WHERE mcid = b.mcid
  ...
  AND product_bridge = b.product_bridge;
DELETE FROM sandbox.PS_modded_cloud AS a USING sandbox.PS_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND a.prior_product_solution = b.prior_product_solution
...
WHERE mcid = b.mcid
  ...
  -- CUSTOMER BRIDGE MODDED Core

  DROP TABLE IF EXISTS sandbox.CB_modded_cloud;
CREATE TABLE sandbox.CB_modded_cloud AS
SELECT *
FROM ufdm.sst_customer_bridge_cloud scb;
ALTER TABLE sandbox.CB_modded_cloud DROP COLUMN id;
...
SELECT a.*,
  ...
  "CB Leftover: Rolled Up Amount LCU",
  "CB Migration: Classification"
FROM sandbox.CB_modded_cloud AS a
...
DELETE FROM sandbox.CB_modded_cloud AS a USING sandbox.CB_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
INSERT INTO sandbox.CB_modded_cloud AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
...
DELETE FROM sandbox.CB_modded_cloud AS a USING sandbox.CB_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
...
WHERE mcid = b.mcid
 ...
 -- CUSTOMER BRIDGE MODDED CORE
DROP TABLE IF EXISTS sandbox.CB_modded_core;
CREATE TABLE sandbox.CB_modded_core AS
SELECT *
FROM ufdm.sst_customer_bridge_core scb;
ALTER TABLE sandbox.CB_modded_core DROP COLUMN id;
DROP TABLE IF EXISTS sandbox.churn_migration_test_cb;
CREATE TABLE sandbox.churn_migration_test_cb AS
SELECT DISTINCT mcid,
  evaluation_period,
  ...
  "CB Migration: Classification",
  "CB Leftover: Classification"
FROM sandbox.churn_migration_main_core
WHERE mcid NOT IN ('-');
DROP TABLE IF EXISTS sandbox.CB_migration_default;
CREATE TABLE sandbox.CB_migration_default AS
SELECT a.*,
  ...
  "CB Leftover: Rolled Up Amount LCU",
  "CB Migration: Classification"
FROM sandbox.CB_modded_core AS a
  JOIN sandbox.churn_migration_test_cb AS b ON a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.currency_code
  AND a.customer_bridge = b.customer_bridge
...
WHERE "CB Migration: Classification" ILIKE ('%migration')
  AND "CB Leftover: Rolled Up Amount" IS NOT NULL;
DELETE FROM sandbox.CB_modded_core AS a USING sandbox.CB_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
INSERT INTO sandbox.CB_modded_core AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
...
WHERE mcid = b.mcid
  ...
  AND baseline_currency = b.baseline_currency
  AND customer_bridge = b.customer_bridge;
DELETE FROM sandbox.CB_modded_core AS a USING sandbox.CB_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
