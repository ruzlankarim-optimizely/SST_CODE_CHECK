...
select *
from ufdm.sst_customer_bridge_cloud
where customer_bridge IN ('Churn', 'Downgrade', 'New', 'Up Sell');


DROP TABLE IF EXISTS sandbox.churn_migration_main_cloud;
create table sandbox.churn_migration_main_cloud as (
    with initial_table as (
        select *,
            ...
            end as prior_product_family_class --    case
    ...
    initial_table_2 as (
        select *,
            ...
            case
                when current_product_family_class = 'Legacy'
            ...
            case
                when prior_product_family_class = 'Legacy'
                and product_arr_change_ccfx < 0
                and prior_period_product_arr_usd_ccfx > 0
                and current_period_product_arr_usd_ccfx = 0 then 1
                else 0
            ...
            case
                when current_product_family_class = 'Named'
                and product_arr_change_ccfx > 0
                and prior_period_product_arr_usd_ccfx = 0
                and current_period_product_arr_usd_ccfx > 0 then 1
                else 0
    ...
    initial_table_3 as (
        select *,
            ...
            case
                --ai)
                when "Downgraded a Legacy Product in Current Date" = 1 --Downgraded a legacy product
                and (
                    sum("Named Product in Current Date with ARR") over(
                        partition by mcid,
                        evaluation_period,
                        currency_code
                    )
                ) > 0 -- have named product in current date with ARR > 0
                then 'downgrade - migration' --ai)
                when "Churned a Legacy Product in Current Date" = 1 --Churned a legacy product
                and (
                    sum("Named Product in Current Date with ARR") over(
                        partition by mcid,
                        evaluation_period,
                ...
                then 'churn - migration' --bi)
                when "Added a Named Product in Current Date" = 1 --Added a named product in the current snapshot date
                and (
                    (
                        sum("Downgraded a Legacy Product in Current Date") over(
                            partition by mcid,
                            evaluation_period,
                            currency_code
                        )
                    ...
                    or (
                        sum("Churned a Legacy Product in Current Date") over(
                            partition by mcid,
                            evaluation_period,
                            currency_code
                        )
                ...
                then 'crosssell - migration' --bii)
                when "Increased a Named Product in Current Date" = 1 --Increased a named product in the current snapshot date
                and (
                    (
                        sum("Downgraded a Legacy Product in Current Date") over(
                            partition by mcid,
                ...
                then 'upsell - migration' --If a customer increased or added a named product & they had a legacy product in the prior snapshot period > 0 ARR we would classify the movement as migration
                --ci)
                when "Added a Named Product in Current Date" = 1 --Added a named product in the current snapshot date
                and (
                    sum("Legacy Product in Previous Date with ARR") over(
                        partition by mcid,
                        evaluation_period,
                        currency_code
                    )
                ) > 0 -- had a legacy product in previous snapshot date with ARR > 0
                then 'crosssell - migration' --cii)
                when "Increased a Named Product in Current Date" = 1 --Added a named product in the current snapshot date
                and (
                    sum("Legacy Product in Previous Date with ARR") over(
                        partition by mcid,
                        evaluation_period,
                ...
                then 'upsell - migration' --di)
                when "Churned a Legacy Product in Current Date" = 1 --Churned a legacy product from prior snapshot date
                and (
                    (
                        sum("Added a Named Product in Current Date") over(
                            partition by mcid,
                            evaluation_period,
                            currency_code
                        )
                    ...
                    or (
                        sum("Increased a Named Product in Current Date") over(
                            partition by mcid,
                            evaluation_period,
                            currency_code
                        )
                ...
                then 'churn - migration' --dii)
                when "Downgraded a Legacy Product in Current Date" = 1 --Downgraded a legacy product from prior snapshot date
                and (
                    (
                        sum("Added a Named Product in Current Date") over(
                            partition by mcid,
