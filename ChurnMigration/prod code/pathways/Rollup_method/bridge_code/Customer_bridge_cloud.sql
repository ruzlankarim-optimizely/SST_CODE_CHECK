DROP TABLE IF EXISTS sandbox.sst_customer_bridge_rollover_cm_cloud;
CREATE TABLE sandbox.sst_customer_bridge_rollover_cm_cloud AS
SELECT *
FROM ufdm_archive.sst_customer_bridge_cloud_lcoked_17122024_0238 scb;
...
-- where a.mcid = 'd74620b9-768f-dd11-a26e-0018717a8c82'
-- and a.evaluation_period = '2023M03'
)
...
select
    ...
    and (counting_migration <> count_movements )
    and abs(total_migration_amount_ccfx) > abs(customer_arr_change_ccfx)
    then Case when (migration_rolled_up_amount_lcu/ total_migration_amount_lcu ) * 2 = 1 then round((migration_rolled_up_amount_lcu / total_migration_amount_lcu) * (total_migration_amount_ccfx - customer_arr_change_ccfx))
...
create table sandbox.customer_churn_migration_default_cases as
    ...
    from sandbox.sst_customer_bridge_rollover_cm_cloud    as a
...
Delete from sandbox.sst_customer_bridge_rollover_cm_cloud as a
...
Insert into sandbox.sst_customer_bridge_rollover_cm_cloud as a (
evaluation_period,
    prior_period,
    current_period,
...
GROUP BY 1,
  ...
  12,
  13;
TRUNCATE TABLE sandbox.sst_customer_bridge_rollover_cm_cloud;
INSERT INTO sandbox.sst_customer_bridge_rollover_cm_cloud(
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
