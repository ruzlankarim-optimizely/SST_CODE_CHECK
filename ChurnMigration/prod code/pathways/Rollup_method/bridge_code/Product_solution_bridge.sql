DROP TABLE IF EXISTS sandbox.sst_product_solution_bridge_rollup_cm;
CREATE TABLE sandbox.sst_product_solution_bridge_rollup_cm AS
SELECT *
FROM ufdm_archive.sst_product_bridge_product_solution_lcoked_10122024_1354;

-- mcid = '89c9b1c2-ac43-56ad-2651-a9e0538a6712'
...
create temp table temp_ps_bridge as
    ...
    from sandbox.sst_product_solution_bridge_rollup_cm as a
...
Drop table if exists sandbox.solution_classifier_migration_joiner;
create table sandbox.solution_classifier_migration_joiner as (
with  classifier as (
select
    evaluation_period,
    mcid ,
...
-- where
--     "Movement Classification" is not null
--     and
--     mcid = '1b026b3d-992b-e111-9eb3-0050568d002c'
--     and evaluation_period = '2023M12'
...
, flagging_table as (
    ...
    and (counting_migration <> count_movements )
    and abs(total_migration_amount_ccfx) > abs(ps_arr_change)
    then Case when (migration_rolled_up_amount_lcu/ total_migration_amount_lcu ) * 2 = 1 then round((migration_rolled_up_amount_lcu / total_migration_amount_lcu) * (total_migration_amount_ccfx - ps_arr_change_lcu))
...
drop table if exists sandbox.churn_migration_limitation;
create table sandbox.churn_migration_limitation as
    with base as (
        Select
            mcid ,
            evaluation_period ,
...
Drop table if exists sandbox.solution_churn_migration_default_cases ;
create table sandbox.solution_churn_migration_default_cases as
    Select
        a.*,
        b.product_solutions ,
    ...
    from sandbox.sst_product_solution_bridge_rollup_cm as a
    join sandbox.churn_migration_limitation as b
    on a.evaluation_period = b.evaluation_period
    and a.mcid = b.mcid
    and a.currency_code = b.currency_code
...
Drop table if exists sandbox.solution_churn_migration_split_cases ;
create table sandbox.solution_churn_migration_split_cases as
    Select
        a.*,
        b.product_solutions ,
        b.pathways as migration_pathways ,
...
Delete from sandbox.sst_product_solution_bridge_rollup_cm as a
Using sandbox.solution_churn_migration_default_cases as b
where a.mcid = b.mcid
...
INSERT INTO sandbox.sst_product_solution_bridge_rollup_cm AS a (
    evaluation_period,
    prior_period,
    current_period,
...
FROM sandbox.solution_churn_migration_default_cases AS b
WHERE mcid = b.mcid
...
Delete from sandbox.sst_product_solution_bridge_rollup_cm as a
Using sandbox.solution_churn_migration_split_cases as b
where a.mcid = b.mcid
and a.evaluation_period = b.evaluation_period
and a.currency_code  = b.currency_code
and concat(a.prior_product_solution, '=',a.current_product_solution) = b.product_solutions
...
SELECT evaluation_period,
  ...
  churn_period,
  customer_bridge,
  coalesce(b.pathways , null) as pathways
FROM sandbox.solution_churn_migration_split_cases AS b
WHERE mcid = b.mcid
  AND evaluation_period = b.evaluation_period
  AND currency_code = b.currency_code
  AND concat(prior_product_solution, '=',current_product_solution) = b.product_solutions
...
drop table if exists sandbox.temp_solution_cm_table;
create table sandbox.temp_solution_cm_table as
SELECT evaluation_period,
  prior_period,
  current_period,
  current_end_customer,
...
GROUP BY 1,
  ...
  18,
  19;
TRUNCATE TABLE sandbox.sst_product_solution_bridge_rollup_cm;
INSERT INTO sandbox.sst_product_solution_bridge_rollup_cm(
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
select evaluation_period,
  ...
  customer_bridge,
  pathways
FROM sandbox.temp_solution_cm_table;