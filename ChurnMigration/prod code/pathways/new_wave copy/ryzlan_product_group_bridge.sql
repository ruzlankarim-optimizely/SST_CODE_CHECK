...
ALTER TABLE ryzlan.sst_product_bridge_product_group
ADD pathways VARCHAR(255);

create or replace function ryzlan.sp_populate_sst_product_bridge_product_group(var_period text, run_acquire_customers int) returns void language plpgsql as $$ BEGIN
...
CREATE temp table churn_migration_classifiers_pg as (
  WITH initial_table_4 as (
    SELECT it3.*,
      ...
      split_part(it3."Movement Classification", ' -- ', 1) bridge_part,
      split_part(it3."Movement Classification", ' -- ', 2) pathways_part,
      case
  ...
  double_classification_marker AS (
    SELECT *,
      CASE
        ...
        AND total_migration_amount_ccfx < pg_arr_change THEN (pg_arr_change - total_migration_amount_ccfx) / count_migrations
        ELSE NULL
      END AS new_leftover_value_ccfx,
      CASE
        WHEN "PG Leftover: Classification" IS NOT NULL
        AND count_migrations > 1
        AND total_migration_amount_ccfx < pg_arr_change THEN (pg_arr_change_lcu - total_migration_amount_lcu) / count_migrations
        ELSE NULL
      END AS new_leftover_value_lcu,
      CASE
        WHEN count_migrations > 1
        AND total_migration_amount_ccfx > pg_arr_change THEN (total_migration_amount_ccfx - pg_arr_change) *(
      ...
      CASE
        WHEN count_migrations > 1
        AND total_migration_amount_ccfx > pg_arr_change THEN (total_migration_amount_lcu - pg_arr_change_lcu) *(
          "PG Migration: Rolled Up Amount LCU" / total_migration_amount_ccfx
        )
        ELSE NULL
...
update ryzlan.sst_product_bridge_product_group
