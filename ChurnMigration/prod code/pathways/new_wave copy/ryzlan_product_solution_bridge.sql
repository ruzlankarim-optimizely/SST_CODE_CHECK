create or replace function ryzlan.sp_populate_sst_product_bridge_product_solution(var_period text, run_acquire_customers int) returns void language plpgsql as $$ BEGIN
DELETE from ryzlan.sst_product_bridge_product_solution
...
SELECT snapshot_date,
  ...
  case
    when coalesce(run_acquire_customers, 0) = 1 then acquire_product_solution
...
Create temp table churn_migration_classifiers_pg as (
  ...
  initial_table_8 as (
    select *,
      ...
      end as "PS Leftover: Rolled Up Amount" --Bring in the product solution columns as well -- to roll it up on the PS level
  ...
  initial_table_9 as (
    select *,
      ...
      case
        when "PS Migration: Rolled Up Amount" is not null then "Movement Classification"
        else null
      end as "PS Migration: Classification",
      case
        when "PS Leftover: Rolled Up Amount" is not null then pg_bridge
        else null
...
FROM arr_product_bridge_tmp AS a
  ...
  AND a.current_product_family = b.current_product_solution
  AND a.prior_product_family = b.prior_product_solution
  AND a.product_bridge = b.pg_bridge
WHERE lower("PS Migration: Classification") ILIKE ('%migration%')
...
WHERE a.mcid = b.mcid
  ...
  AND a.current_product_family = b.current_product_family
  AND a.prior_product_family = b.prior_product_family
  AND a.product_bridge = b.product_bridge;
...
WHERE mcid = b.mcid
  ...
  AND current_product_family = b.current_product_family
  AND prior_product_family = b.prior_product_family
...
SELECT evaluation_period,
  ...
  prior_arr_usd_ccfx * abs(
    "PS Migration: Rolled Up Amount" / product_arr_change_ccfx
  ),
  ...
  "PS Migration: Rolled Up Amount",
  prior_arr_lcu * abs(
    "PS Migration: Rolled Up Amount LCU" / product_arr_change_lcu
  ),
  current_arr_lcu * abs(
    "PS Migration: Rolled Up Amount LCU" / product_arr_change_lcu
  ),
...
SELECT evaluation_period,
  ...
  baseline_currency,
  prior_arr_usd_ccfx * abs(
    "PS Leftover: Rolled Up Amount" / product_arr_change_ccfx
  ),
  current_arr_usd_ccfx * abs(
    "PS Leftover: Rolled Up Amount" / product_arr_change_ccfx
  ),
  ...
  "PS Leftover: Rolled Up Amount",
  prior_arr_lcu * abs(
    "PS Leftover: Rolled Up Amount LCU" / product_arr_change_lcu
  ),
  current_arr_lcu * abs(
    "PS Leftover: Rolled Up Amount LCU" / product_arr_change_lcu
  ),
...
INSERT INTO ryzlan.sst_product_bridge_product_solution (
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
--     update ryzlan.sst_product_bridge_product_solution a
...
create temp table temp_win_downgrade_upsell as with temp1 as (
  ...
  from ryzlan.sst_product_bridge_product_solution a
  where 1 = 1
...
temp2 as (
  ...
  from ryzlan.sst_product_bridge_product_solution b
    join temp1 a on a.mcid = b.mcid
    and a.current_product_group = b.current_product_solution
  where 1 = 1
...
from ryzlan.sst_product_bridge_product_solution a
  ...
  and a.evaluation_period = b.evaluation_period
  and a.current_product_solution = b.current_product_group
  left join ryzlan.sst_product_bridge_product_solution c on c.mcid = a.mcid
  and c.evaluation_period = a.evaluation_period
  and c.current_product_solution = a.current_product_solution
  and c.product_bridge = 'Up Sell'
where b.Split_record = 1
  and a.evaluation_period = var_period
  and a.product_bridge in ('Cross-sell')
...
delete from ryzlan.sst_product_bridge_product_solution a using temp_windowngrade_final_curated b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
...
temp2 as (
  ...
  from ryzlan.sst_product_bridge_product_solution b
    join temp1 a on a.mcid = b.mcid
    and a.product_group_at_Downgrade_Churn = b.current_product_solution
  where 1 = 1
...
from ryzlan.sst_product_bridge_product_solution a
  ...
  left join (
    ...
    from ryzlan.sst_product_bridge_product_solution suba
      join ufdm_grey.periods subb on suba.evaluation_period = subb.evaluation_period
  ) c on a.mcid = c.mcid
  and coalesce (
    a.current_product_solution,
    a.prior_product_solution
  ) = c.current_product_solution
...
from ryzlan.sst_product_bridge_product_solution a
  ...
  join ryzlan.sst_product_bridge_product_solution c on c.mcid = b.mcid
  and c.evaluation_period = b.CPI_evaluation_period
  and coalesce(
    c.current_product_solution,
    c.prior_product_solution
  ) = b.current_product_group
...
where b.Split_record = 1
  ...
  and a.product_bridge in ('Downgrade', 'Churn', 'Downsell')
order by mcid;
delete from ryzlan.sst_product_bridge_product_solution a using temp_CPI_Reversal b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and coalesce(
...
temp2 as (
  ...
  from ryzlan.sst_product_bridge_product_solution b
    join temp1 a on a.mcid = b.mcid
    and a.product_group_downgrade = b.current_product_solution
  where 1 = 1
...
where b.Split_record = 1
  ...
  and a.product_bridge in ('Downgrade', 'Downsell')
order by mcid;
delete from ryzlan.sst_product_bridge_product_solution a using temp_upselldowngrade_final b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and a.current_product_solution = b.product_group_downgrade
...
update ryzlan.sst_product_bridge_product_solution
