...
DROP TABLE IF EXISTS sandbox.sst_customer_bridge_core_mig;
CREATE TABLE sandbox.sst_customer_bridge_core_mig AS
SELECT *
FROM ufdm_archive.sst_customer_bridge_core_temp_lcoked_251209_0226
...
with    base_product_group AS (
  ...
  FROM ufdm_archive.sst_pb_ps_cloud_license_mig_lcoked_251209_0226
    where current_product_solution   in ('AI', 'Content Supply Chain', 'Experimentation & Personalization (E&P)')
    or prior_product_solution  in ('AI', 'Content Supply Chain', 'Experimentation & Personalization (E&P)')
),
...
create table sandbox.customer_churn_migration_default_cases as
    ...
    from sandbox.sst_customer_bridge_core_mig    as a
...
Delete from sandbox.sst_customer_bridge_core_mig as a
...
Insert into sandbox.sst_customer_bridge_core_mig as a (
evaluation_period,
    prior_period,
    current_period,
...
FROM sandbox.sst_customer_bridge_core_mig
GROUP BY 1,
  2,
  3,
  4,
  ...
  12,
  13;
TRUNCATE TABLE sandbox.sst_customer_bridge_core_mig;
INSERT INTO sandbox.sst_customer_bridge_core_mig(
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
