...
DROP TABLE IF EXISTS sandbox.sst_customer_bridge_non_ai_mig;
CREATE TABLE sandbox.sst_customer_bridge_non_ai_mig AS
SELECT *
FROM ufdm_archive.sst_customer_bridge_non_ai_temp_lcoked_251209_0226
...
with    base_product_group AS (
  ...
  where current_product_solution   in ('Content Supply Chain', 'Experimentation & Personalization (E&P)', 'EOL', 'Other', 'License')
    or prior_product_solution  in ('Content Supply Chain', 'Experimentation & Personalization (E&P)', 'EOL', 'Other', 'License')
),
...
create table sandbox.customer_churn_migration_default_cases as
    ...
    from sandbox.sst_customer_bridge_non_ai_mig    as a
...
Delete from sandbox.sst_customer_bridge_non_ai_mig as a
...
Insert into sandbox.sst_customer_bridge_non_ai_mig as a (
evaluation_period,
    prior_period,
    current_period,
...
FROM sandbox.sst_customer_bridge_non_ai_mig
GROUP BY 1,
  2,
  3,
  4,
  ...
  12,
  13;
TRUNCATE TABLE sandbox.sst_customer_bridge_non_ai_mig;
INSERT INTO sandbox.sst_customer_bridge_non_ai_mig(
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
