...
--     select count(*) from initial_table ;
  , initial_table_2 as (
    ...
    *,
    ...
    case when current_pathways = 'Search & Navigation - Standalone Subscription'
    and product_arr_change_ccfx < 0
    and prior_period_product_arr_usd_ccfx > 0
    and current_period_product_arr_usd_ccfx > 0 then 1
    else 0 end as downgraded_search_nav_subs_in_current_date ,
    ...
    case when prior_pathways = 'Search & Navigation - Standalone Subscription'
    and product_arr_change_ccfx < 0
    and prior_period_product_arr_usd_ccfx > 0
    and current_period_product_arr_usd_ccfx = 0 then 1
    else 0 end as churned_search_nav_subs_in_current_date,
    ...
    case when current_pathways = 'Content Supply Chain'
    and product_arr_change_ccfx > 0
    and prior_period_product_arr_usd_ccfx = 0
    and current_period_product_arr_usd_ccfx > 0 then 1
    else 0 end as added_content_supply_chain_in_current_date,
    ...
    and prior_period_product_arr_usd_ccfx > 0
    and current_period_product_arr_usd_ccfx > 0 then 1
    else 0 end as increased_content_supply_chain_in_current_date,
    case when current_pathways= 'Segmentation'
    and product_arr_change_ccfx > 0
    and prior_period_product_arr_usd_ccfx > 0
    and current_period_product_arr_usd_ccfx > 0 then 1
    ...
    case when current_pathways = 'Content Supply Chain'
    and current_period_product_arr_usd_ccfx > 0 then 1
    else 0 end as content_supply_chain_in_current_date_with_arr,
    case when current_pathways = 'Segmentation'
    and current_period_product_arr_usd_ccfx > 0 then 1
    ...
    case when prior_pathways = 'Search & Navigation - Standalone Subscription'
    and prior_period_product_arr_usd_ccfx > 0 then 1
    else 0 end as search_nav_subs_in_previous_date_with_arr,
    from initial_table
  )
  , initial_table_3 as (
    select
    *,
    ...
    sum(content_supply_chain_in_current_date_with_arr)over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_content_supply_chain_in_current_date_with_arr,
    ...
    sum(downgraded_search_navigation_ms_in_current_date) over(
      ...
      sum(downgraded_search_nav_subs_in_current_date) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_downgraded_search_nav_subs_in_current_date,
    -- SUM CHURNED
    ...
    sum(churned_search_navigation_ms_in_current_date) over(
      ...
      sum(churned_search_nav_subs_in_current_date) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_churned_search_nav_subs_in_current_date,
    ...
    sum(search_navigation_standalone_ms_in_previous_date_with_arr) over(
      ...
      sum(search_nav_subs_in_previous_date_with_arr) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_search_nav_subs_in_previous_date_with_arr,

    -- SUM ADDED in current date
    sum(added_other_in_current_date) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_added_other_in_current_date,
    sum(added_content_supply_chain_in_current_date) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_added_content_supply_chain_in_current_date,
    ...
    sum(increased_content_supply_chain_in_current_date) over(
      partition by mcid,evaluation_period,currency_code
      ) as sum_increased_content_supply_chain_in_current_date,
  ...
  ,  initial_table_4 as (
    ...
    *,
    ...
    case when downgraded_commerce_connect_li_in_current_date= 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_commerce_connect_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_commerce_connect_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_configured_commerce_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_configured_commerce_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_content_management_paas_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_content_management_paas_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_eol_everweb_to_content_supply_chain,
    ...
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_legacy_ektron_to_other,

    case when downgraded_legacy_ektron_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_legacy_ektron_to_content_supply_chain,
    -- downgrade legacy ektron ms to product
    case when downgraded_legacy_ektron_ms_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_legacy_ektron_ms_to_other,

    case when downgraded_legacy_ektron_ms_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_legacy_ektron_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_search_navigation_to_content_supply_chain,
    -- downgrade search navigation standalone ms to product
    case when downgraded_search_navigation_ms_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_search_navigation_ms_to_other,

    case when downgraded_search_navigation_ms_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_search_navigation_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_configured_commerce_subs_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_eol_community_api_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_commerce_connect_subs_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_content_mgmt_paas_subs_to_content_supply_chain,

    -- downgrade Search Nav Standalone Subs to product
    case when downgraded_search_nav_subs_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_search_nav_subs_to_other,

    case when downgraded_search_nav_subs_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downgrade_migration_search_nav_subs_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_commerce_connect_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_commerce_connect_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_configured_commerce_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_configured_commerce_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_content_management_paas_li_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_content_management_paas_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_eol_everweb_to_content_supply_chain,
    ...
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_legacy_ektron_to_other,
    case when churned_legacy_ektron_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_legacy_ektron_to_content_supply_chain,
    -- churned legacy ektron ms to product
    case when churned_legacy_ektron_ms_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_legacy_ektron_ms_to_other,
    case when churned_legacy_ektron_ms_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_legacy_ektron_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_search_navigation_to_content_supply_chain,
    -- churned search navigation standalone ms to product
    case when churned_search_navigation_ms_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_search_navigation_ms_to_other,
    case when churned_search_navigation_ms_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_search_navigation_ms_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_configured_commerce_subs_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_community_api_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_commerce_connect_subs_to_content_supply_chain,
    ...
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_content_mgmt_paas_subs_to_content_supply_chain,

    -- churned Search NAV standalone Subscriptions
    case when churned_search_nav_subs_in_current_date = 1 then
    case when sum_other_in_current_date_with_arr > 0 then 1
    when sum_added_other_in_current_date > 0 or sum_increased_other_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_search_nav_subs_to_other,
    case when churned_search_nav_subs_in_current_date = 1 then
    case when sum_content_supply_chain_in_current_date_with_arr > 0 then 1
    when sum_added_content_supply_chain_in_current_date > 0 or sum_increased_content_supply_chain_in_current_date > 0
    then 1 else 0 end
    else 0 end as downsell_migration_search_nav_subs_to_content_supply_chain,
    ...
    then 1 else 0 end
    else 0 end as cross_sell_migration_legacy_ektron_to_other,
    case when added_other_in_current_date=1 then
    ...
    then 1 else 0 end
    else 0 end as cross_sell_migration_legacy_ektron_ms_to_other,
    case when added_other_in_current_date=1 then
    ...
    then 1 else 0 end
    else 0 end as cross_sell_migration_search_navigation_ms_to_other,
    case when added_other_in_current_date=1 then
    ...
    case when added_other_in_current_date=1 then
    case when sum_downgraded_search_nav_subs_in_current_date > 0
    or sum_churned_search_nav_subs_in_current_date > 0 then 1
    when sum_search_nav_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_search_nav_subs_to_other,

    -- cross sell content management cms to migration from products
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_commerce_connect_li_in_current_date > 0
    or sum_churned_commerce_connect_li_in_current_date > 0 then 1
    when sum_commerce_connect_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_commerce_connect_li_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_commerce_connect_ms_in_current_date > 0
    or sum_churned_commerce_connect_ms_in_current_date > 0 then 1
    when sum_commerce_connect_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_commerce_connect_ms_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_configured_commerce_li_in_current_date > 0
    or sum_churned_configured_commerce_li_in_current_date > 0 then 1
    when sum_configured_commerce_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_configured_commerce_li_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_configured_commerce_ms_in_current_date > 0
    or sum_churned_configured_commerce_ms_in_current_date > 0 then 1
    when sum_configured_commerce_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_configured_commerce_ms_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_content_management_paas_li_in_current_date > 0
    or sum_churned_content_management_paas_li_in_current_date > 0 then 1
    when sum_content_management_paas_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_content_management_paas_li_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_content_management_paas_ms_in_current_date > 0
    or sum_churned_content_management_paas_ms_in_current_date > 0 then 1
    when sum_content_management_paas_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_content_management_paas_ms_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_eol_everweb_in_current_date > 0
    or sum_churned_eol_everweb_in_current_date > 0 then 1
    when sum_eol_everweb_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_eol_everweb_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_legacy_ektron_in_current_date > 0
    or sum_churned_legacy_ektron_in_current_date > 0 then 1
    when sum_legacy_ektron_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_legacy_ektron_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_legacy_ektron_ms_in_current_date > 0
    or sum_churned_legacy_ektron_ms_in_current_date > 0 then 1
    when sum_legacy_ektron_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_legacy_ektron_ms_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_search_navigation_in_current_date > 0
    or sum_churned_search_navigation_in_current_date > 0 then 1
    when sum_search_navigation_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_search_navigation_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_search_navigation_ms_in_current_date > 0
    or sum_churned_search_navigation_ms_in_current_date > 0 then 1
    when sum_search_navigation_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_search_navigation_ms_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_configured_commerce_subs_in_current_date > 0
    or sum_churned_configured_commerce_subs_in_current_date > 0 then 1
    when sum_configured_commerce_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_configured_commerce_subs_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_eol_community_api_in_current_date > 0
    or sum_churned_community_api_in_current_date > 0 then 1
    when sum_community_api_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_eol_community_api_to_content_supply_chain,
    ...
    when sum_commerce_connect_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_commerce_connect_subs_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_content_mgmt_paas_subs_in_current_date > 0
    or sum_churned_content_mgmt_paas_subs_in_current_date > 0 then 1
    when sum_content_mgmt_paas_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_content_mgmt_paas_subs_to_content_supply_chain,
    case when added_content_supply_chain_in_current_date=1 then
    case when sum_downgraded_search_nav_subs_in_current_date > 0
    or sum_churned_search_nav_subs_in_current_date > 0 then 1
    when sum_search_nav_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as cross_sell_migration_search_nav_subs_to_content_supply_chain,
    ...
    when sum_legacy_ektron_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_legacy_ektron_to_other,
    case when increased_other_in_current_date = 1 then
    case when sum_downgraded_legacy_ektron_ms_in_current_date > 0
    or sum_churned_legacy_ektron_ms_in_current_date > 0 then 1
    when sum_legacy_ektron_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_legacy_ektron_ms_to_other,
    case when increased_other_in_current_date = 1 then
    case when sum_downgraded_search_navigation_in_current_date > 0
    or sum_churned_search_navigation_in_current_date > 0 then 1
    when sum_search_navigation_in_previous_date_with_arr > 0
    ...
    when sum_search_navigation_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_search_navigation_ms_to_other,
    case when increased_other_in_current_date = 1 then
    case when sum_downgraded_configured_commerce_subs_in_current_date > 0
    or sum_churned_configured_commerce_subs_in_current_date > 0 then 1
    ...
    when sum_search_nav_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_search_nav_subs_to_other,


    -- upsell migration content management cms to migration from products
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_commerce_connect_li_in_current_date > 0
    or sum_churned_commerce_connect_li_in_current_date > 0 then 1
    when sum_commerce_connect_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_commerce_connect_li_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_commerce_connect_ms_in_current_date > 0
    or sum_churned_commerce_connect_ms_in_current_date > 0 then 1
    when sum_commerce_connect_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_commerce_connect_ms_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_configured_commerce_li_in_current_date > 0
    or sum_churned_configured_commerce_li_in_current_date > 0 then 1
    when sum_configured_commerce_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_configured_commerce_li_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_configured_commerce_ms_in_current_date > 0
    or sum_churned_configured_commerce_ms_in_current_date > 0 then 1
    when sum_configured_commerce_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_configured_commerce_ms_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_content_management_paas_li_in_current_date > 0
    or sum_churned_content_management_paas_li_in_current_date > 0 then 1
    when sum_content_management_paas_li_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_content_management_paas_li_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_content_management_paas_ms_in_current_date > 0
    or sum_churned_content_management_paas_ms_in_current_date > 0 then 1
    when sum_content_management_paas_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_content_management_paas_ms_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_eol_everweb_in_current_date > 0
    or sum_churned_eol_everweb_in_current_date > 0 then 1
    when sum_eol_everweb_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_eol_everweb_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_legacy_ektron_in_current_date > 0
    or sum_churned_legacy_ektron_in_current_date > 0 then 1
    when sum_legacy_ektron_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_legacy_ektron_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_legacy_ektron_ms_in_current_date > 0
    or sum_churned_legacy_ektron_ms_in_current_date > 0 then 1
    when sum_legacy_ektron_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_legacy_ektron_ms_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_search_navigation_in_current_date > 0
    or sum_churned_search_navigation_in_current_date > 0 then 1
    when sum_search_navigation_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_search_navigation_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_search_navigation_ms_in_current_date > 0
    or sum_churned_search_navigation_ms_in_current_date > 0 then 1
    when sum_search_navigation_ms_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_search_navigation_ms_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_configured_commerce_subs_in_current_date > 0
    or sum_churned_configured_commerce_subs_in_current_date > 0 then 1
    when sum_configured_commerce_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_configured_commerce_subs_to_content_supply_chain,
    case when increased_content_supply_chain_in_current_date = 1 then
    case when sum_downgraded_eol_community_api_in_current_date > 0
    or sum_churned_community_api_in_current_date > 0 then 1
    when sum_community_api_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_community_api_to_content_supply_chain,
    ...
    when sum_commerce_connect_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_commerce_connect_subs_to_content_supply_chain,
    ...
    when sum_content_mgmt_paas_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_content_mgmt_paas_subs_to_content_supply_chain,
    ...
    when sum_search_nav_subs_in_previous_date_with_arr > 0
    then 1 else 0 end
    else 0 end as upsell_migration_search_nav_subs_to_content_supply_chain,
...
--
--          downgraded_visitor_intelligence_in_current_date,
--          churned_visitor_intelligence_in_current_date,
--          eol_legacy_visitor_intelligence_in_previous_date_with_arr,
--
--          added_segmentation_in_current_date,
--          increased_segmentation_in_current_date,
--          segmentation_in_current_date_with_arr ,
--
--          sum_segmentation_in_current_date_with_arr,
--          sum_added_segmentation_in_current_date,
--          sum_increased_segmentation_in_current_date ,
--
--          sum_downgraded_visitor_intelligence_in_current_date ,
--          sum_churned_visitor_intelligence_in_current_date,
--          sum_visitor_intelligence_in_previous_date_with_arr,
--
--          downsell_migration_visitor_intelligence_to_segmentation,
--          cross_sell_migration_visitor_intelligence_to_segmentation
--
--
--      from initial_table_4 ;
  ...
  , initial_table_5 as (
    ...
    * ,
    ...
    downgrade_migration_legacy_ektron_to_other +
    downgrade_migration_legacy_ektron_ms_to_other +
    downgrade_migration_search_navigation_to_other +
    downgrade_migration_search_navigation_ms_to_other +
    ...
    downgrade_migration_search_nav_subs_to_other +


    downgrade_migration_commerce_connect_li_to_content_supply_chain +
    downgrade_migration_commerce_connect_ms_to_content_supply_chain +
    downgrade_migration_configured_commerce_li_to_content_supply_chain +
    downgrade_migration_configured_commerce_ms_to_content_supply_chain +
    downgrade_migration_content_management_paas_li_to_content_supply_chain +
    downgrade_migration_content_management_paas_ms_to_content_supply_chain +
    downgrade_migration_eol_everweb_to_content_supply_chain +
    downgrade_migration_legacy_ektron_to_content_supply_chain +
    downgrade_migration_legacy_ektron_ms_to_content_supply_chain +
    downgrade_migration_search_navigation_to_content_supply_chain +
    downgrade_migration_search_navigation_ms_to_content_supply_chain +
    downgrade_migration_configured_commerce_subs_to_content_supply_chain +
    downgrade_migration_eol_community_api_to_content_supply_chain +
    downgrade_migration_commerce_connect_subs_to_content_supply_chain +
    downgrade_migration_content_mgmt_paas_subs_to_content_supply_chain + 
    downgrade_migration_search_nav_subs_to_content_supply_chain + 
    ...
    downsell_migration_legacy_ektron_to_other +
    downsell_migration_legacy_ektron_ms_to_other +
    downsell_migration_search_navigation_to_other +
    downsell_migration_search_navigation_ms_to_other +
    ...
    downsell_migration_search_nav_subs_to_other + 


    downsell_migration_commerce_connect_li_to_content_supply_chain+
    downsell_migration_commerce_connect_ms_to_content_supply_chain+
    downsell_migration_configured_commerce_li_to_content_supply_chain+
    downsell_migration_configured_commerce_ms_to_content_supply_chain+
    downsell_migration_content_management_paas_li_to_content_supply_chain+
    downsell_migration_content_management_paas_ms_to_content_supply_chain+
    downsell_migration_eol_everweb_to_content_supply_chain+
    downsell_migration_legacy_ektron_to_content_supply_chain+
    downsell_migration_legacy_ektron_ms_to_content_supply_chain+
    downsell_migration_search_navigation_to_content_supply_chain+
    downsell_migration_search_navigation_ms_to_content_supply_chain+
    downsell_migration_configured_commerce_subs_to_content_supply_chain +
    downsell_migration_community_api_to_content_supply_chain +
    downsell_migration_commerce_connect_subs_to_content_supply_chain + 
    downsell_migration_content_mgmt_paas_subs_to_content_supply_chain + 
    downsell_migration_search_nav_subs_to_content_supply_chain +
    ...
    cross_sell_migration_legacy_ektron_to_other +
    cross_sell_migration_legacy_ektron_ms_to_other +
    cross_sell_migration_search_navigation_to_other +
    cross_sell_migration_search_navigation_ms_to_other +
    ...
    cross_sell_migration_search_nav_subs_to_other +

    cross_sell_migration_commerce_connect_li_to_content_supply_chain +
    cross_sell_migration_commerce_connect_ms_to_content_supply_chain +
    cross_sell_migration_configured_commerce_li_to_content_supply_chain +
    cross_sell_migration_configured_commerce_ms_to_content_supply_chain +
    cross_sell_migration_content_management_paas_li_to_content_supply_chain +
    cross_sell_migration_content_management_paas_ms_to_content_supply_chain +
    cross_sell_migration_eol_everweb_to_content_supply_chain +
    cross_sell_migration_legacy_ektron_to_content_supply_chain +
    cross_sell_migration_legacy_ektron_ms_to_content_supply_chain +
    cross_sell_migration_search_navigation_to_content_supply_chain +
    cross_sell_migration_search_navigation_ms_to_content_supply_chain +
    cross_sell_migration_configured_commerce_subs_to_content_supply_chain +
    cross_sell_migration_eol_community_api_to_content_supply_chain +
    cross_sell_migration_commerce_connect_subs_to_content_supply_chain + 
    cross_sell_migration_content_mgmt_paas_subs_to_content_supply_chain +
    cross_sell_migration_search_nav_subs_to_content_supply_chain +
    ...
    upsell_migration_legacy_ektron_to_other +
    upsell_migration_legacy_ektron_ms_to_other +
    upsell_migration_search_navigation_to_other +
    upsell_migration_search_navigation_ms_to_other +
    ...
    upsell_migration_search_nav_subs_to_other +

    upsell_migration_commerce_connect_li_to_content_supply_chain +
    upsell_migration_commerce_connect_ms_to_content_supply_chain +
    upsell_migration_configured_commerce_li_to_content_supply_chain +
    upsell_migration_configured_commerce_ms_to_content_supply_chain +
    upsell_migration_content_management_paas_li_to_content_supply_chain +
    upsell_migration_content_management_paas_ms_to_content_supply_chain +
    upsell_migration_eol_everweb_to_content_supply_chain +
    upsell_migration_legacy_ektron_to_content_supply_chain +
    upsell_migration_legacy_ektron_ms_to_content_supply_chain +
    upsell_migration_search_navigation_to_content_supply_chain +
    upsell_migration_search_navigation_ms_to_content_supply_chain +
    upsell_migration_configured_commerce_subs_to_content_supply_chain +
    upsell_migration_community_api_to_content_supply_chain + 
    upsell_migration_commerce_connect_subs_to_content_supply_chain +
    upsell_migration_content_mgmt_paas_subs_to_content_supply_chain +
    upsell_migration_search_nav_subs_to_content_supply_chain +
    ...
    case when downgrade_migration_legacy_ektron_to_other = 1 then
    ',downgrade - migration -- legacy_ektron to other' else null end,
    case when downgrade_migration_legacy_ektron_ms_to_other = 1 then
    ',downgrade - migration -- legacy_ektron_ms to other' else null end,
    ...
    case when downgrade_migration_search_navigation_ms_to_other = 1 then
    ',downgrade - migration -- search_navigation_standalone_ms to other' else null end,
    ...
    case when downgrade_migration_search_nav_subs_to_other = 1 then
    ',downgrade - migration -- search_navigation_standalone_subscriptions to other' else null end,

    case when downgrade_migration_commerce_connect_li_to_content_supply_chain = 1 then
    ',downgrade - migration -- commerce_connect_li to content_supply_chain' else null end,
    case when downgrade_migration_commerce_connect_ms_to_content_supply_chain = 1 then
    ',downgrade - migration -- commerce_connect_ms to content_supply_chain' else null end,
    case when downgrade_migration_configured_commerce_li_to_content_supply_chain = 1 then
    ',downgrade - migration -- configured_commerce_li to content_supply_chain' else null end,
    case when downgrade_migration_configured_commerce_ms_to_content_supply_chain = 1 then
    ',downgrade - migration -- configured_commerce_ms to content_supply_chain' else null end,
    case when downgrade_migration_content_management_paas_li_to_content_supply_chain = 1 then
    ',downgrade - migration -- content_management_paas_li to content_supply_chain' else null end,
    case when downgrade_migration_content_management_paas_ms_to_content_supply_chain = 1 then
    ',downgrade - migration -- content_management_paas_ms to content_supply_chain' else null end,
    case when downgrade_migration_eol_everweb_to_content_supply_chain = 1 then
    ',downgrade - migration -- eol_everweb to content_supply_chain' else null end,
    case when downgrade_migration_legacy_ektron_to_content_supply_chain = 1 then
    ',downgrade - migration -- legacy_ektron to content_supply_chain' else null end,
    case when downgrade_migration_legacy_ektron_ms_to_content_supply_chain = 1 then
    ',downgrade - migration -- legacy_ektron_ms to content_supply_chain' else null end,
    case when downgrade_migration_search_navigation_to_content_supply_chain = 1 then
    ',downgrade - migration -- search_navigation_standalone to content_supply_chain' else null end,
    case when downgrade_migration_search_navigation_ms_to_content_supply_chain = 1 then
    ',downgrade - migration -- search_navigation_standalone_ms to content_supply_chain' else null end,
    case when downgrade_migration_configured_commerce_subs_to_content_supply_chain = 1 then
    ',downgrade - migration -- configured_commerce_subs to content_supply_chain' else null end,
    case when downgrade_migration_eol_community_api_to_content_supply_chain = 1 then
    ',downgrade - migration -- eol_community_api to content_supply_chain' else null end,

    case when downgrade_migration_commerce_connect_subs_to_content_supply_chain = 1 then
    ',downgrade - migration -- commerce_connect_subscriptions to content_supply_chain' else null end,
    case when downgrade_migration_content_mgmt_paas_subs_to_content_supply_chain = 1 then
    ',downgrade - migration -- content_management_paas_subscriptions to content_supply_chain' else null end,
    case when downgrade_migration_search_nav_subs_to_content_supply_chain = 1 then
    ',downgrade - migration -- search_navigation_standalone_subscriptions to content_supply_chain' else null end,
    ...
    case when downsell_migration_legacy_ektron_to_other = 1 then
    ',downsell - migration -- legacy_ektron to other' else null end,
    case when downsell_migration_legacy_ektron_ms_to_other = 1 then
    ',downsell - migration -- legacy_ektron_ms to other' else null end,
    ...
    case when downsell_migration_search_navigation_ms_to_other = 1 then
    ',downsell - migration -- search_navigation_standalone_ms to other' else null end,
    ...
    case when downsell_migration_search_nav_subs_to_other = 1 then
    ',downsell - migration -- search_navigation_standalone_subscriptions to other' else null end,

    case when downsell_migration_commerce_connect_li_to_content_supply_chain = 1 then
    ',downsell - migration -- commerce_connect_li to content_supply_chain' else null end,
    case when downsell_migration_commerce_connect_ms_to_content_supply_chain = 1 then
    ',downsell - migration -- commerce_connect_ms to content_supply_chain' else null end,
    case when downsell_migration_configured_commerce_li_to_content_supply_chain = 1 then
    ',downsell - migration -- configured_commerce_li to content_supply_chain' else null end,
    case when downsell_migration_configured_commerce_ms_to_content_supply_chain = 1 then
    ',downsell - migration -- configured_commerce_ms to content_supply_chain' else null end,
    case when downsell_migration_content_management_paas_li_to_content_supply_chain = 1 then
    ',downsell - migration -- content_management_paas_li to content_supply_chain' else null end,
    case when downsell_migration_content_management_paas_ms_to_content_supply_chain = 1 then
    ',downsell - migration -- content_management_paas_ms to content_supply_chain' else null end,
    case when downsell_migration_eol_everweb_to_content_supply_chain = 1 then
    ',downsell - migration -- eol_everweb to content_supply_chain' else null end,
    case when downsell_migration_legacy_ektron_to_content_supply_chain = 1 then
    ',downsell - migration -- legacy_ektron to content_supply_chain' else null end,
    case when downsell_migration_legacy_ektron_ms_to_content_supply_chain = 1 then
    ',downsell - migration -- legacy_ektron_ms to content_supply_chain' else null end,
    case when downsell_migration_search_navigation_to_content_supply_chain = 1 then
    ',downsell - migration -- search_navigation_standalone to content_supply_chain' else null end,
    case when downsell_migration_search_navigation_ms_to_content_supply_chain = 1 then
    ',downsell - migration -- search_navigation_standalone_ms to content_supply_chain' else null end,
    case when downsell_migration_configured_commerce_subs_to_content_supply_chain = 1 then
    ',downsell - migration -- configured_commerce_subs to content_supply_chain' else null end,
    case when downsell_migration_community_api_to_content_supply_chain = 1 then
    ',downsell - migration -- eol_community_api to content_supply_chain' else null end,

    case when downsell_migration_commerce_connect_subs_to_content_supply_chain = 1 then
    ',downsell - migration -- commerce_connect_subscriptions to content_supply_chain' else null end,
    case when downsell_migration_content_mgmt_paas_subs_to_content_supply_chain = 1 then
    ',downsell - migration -- content_management_paas_subscriptions to content_supply_chain' else null end,
    case when downsell_migration_search_nav_subs_to_content_supply_chain = 1 then
    ',downsell - migration -- search_navigation_standalone_subscriptions to content_supply_chain' else null end,
    ...
    case when cross_sell_migration_legacy_ektron_to_other = 1 then
    ',cross_sell - migration -- legacy_ektron to other' else null end,
    case when cross_sell_migration_legacy_ektron_ms_to_other = 1 then
    ',cross_sell - migration -- legacy_ektron_ms to other' else null end,
    ...
    case when cross_sell_migration_search_navigation_ms_to_other = 1 then
    ',cross_sell - migration -- search_navigation_standalone_ms to other' else null end,
    ...
    case when cross_sell_migration_search_nav_subs_to_other = 1 then
    ',cross_sell - migration -- search_navigation_standalone_subscriptions to other' else null end,

    case when cross_sell_migration_commerce_connect_li_to_content_supply_chain = 1 then
    ',cross_sell - migration -- commerce_connect_li to content_supply_chain' else null end,
    case when cross_sell_migration_commerce_connect_ms_to_content_supply_chain = 1 then
    ',cross_sell - migration -- commerce_connect_ms to content_supply_chain' else null end,
    case when cross_sell_migration_configured_commerce_li_to_content_supply_chain = 1 then
    ',cross_sell - migration -- configured_commerce_li to content_supply_chain' else null end,
    case when cross_sell_migration_configured_commerce_ms_to_content_supply_chain = 1 then
    ',cross_sell - migration -- configured_commerce_ms to content_supply_chain' else null end,
    case when cross_sell_migration_content_management_paas_li_to_content_supply_chain = 1 then
    ',cross_sell - migration -- content_management_paas_li to content_supply_chain' else null end,
    case when cross_sell_migration_content_management_paas_ms_to_content_supply_chain = 1 then
    ',cross_sell - migration -- content_management_paas_ms to content_supply_chain' else null end,
    case when cross_sell_migration_eol_everweb_to_content_supply_chain = 1 then
    ',cross_sell - migration -- eol_everweb to content_supply_chain' else null end,
    case when cross_sell_migration_legacy_ektron_to_content_supply_chain = 1 then
    ',cross_sell - migration -- legacy_ektron to content_supply_chain' else null end,
    case when cross_sell_migration_legacy_ektron_ms_to_content_supply_chain = 1 then
    ',cross_sell - migration -- legacy_ektron_ms to content_supply_chain' else null end,
    case when cross_sell_migration_search_navigation_to_content_supply_chain = 1 then
    ',cross_sell - migration -- search_navigation_standalone to content_supply_chain' else null end,
    case when cross_sell_migration_search_navigation_ms_to_content_supply_chain = 1 then
    ',cross_sell - migration -- search_navigation_standalone_ms to content_supply_chain' else null end,
    case when cross_sell_migration_configured_commerce_subs_to_content_supply_chain = 1 then
    ',cross_sell - migration -- configured_commerce_subs to content_supply_chain' else null end,
    case when cross_sell_migration_eol_community_api_to_content_supply_chain = 1 then
    ',cross_sell - migration -- eol_community_api to content_supply_chain' else null end,

    case when cross_sell_migration_commerce_connect_subs_to_content_supply_chain = 1 then
    ',cross_sell - migration -- commerce_connect_subscriptions to content_supply_chain' else null end,
    case when cross_sell_migration_content_mgmt_paas_subs_to_content_supply_chain = 1 then
    ',cross_sell - migration -- content_management_paas_subscriptions to content_supply_chain' else null end,
    case when cross_sell_migration_search_nav_subs_to_content_supply_chain = 1 then
    ',cross_sell - migration -- search_navigation_standalone_subscriptions to content_supply_chain' else null end,
    ...
    case when upsell_migration_legacy_ektron_to_other = 1 then
    ',upsell - migration -- legacy_ektron to other' else null end,
    case when upsell_migration_legacy_ektron_ms_to_other = 1 then
    ',upsell - migration -- legacy_ektron_ms to other' else null end,
    ...
    case when upsell_migration_search_navigation_ms_to_other = 1 then
    ',upsell - migration -- search_navigation_standalone_ms to other' else null end,
    ...
    case when upsell_migration_search_nav_subs_to_other = 1 then
    ',upsell - migration -- search_navigation_standalone_subscriptions to other' else null end,


    case when upsell_migration_commerce_connect_li_to_content_supply_chain = 1 then
    ',upsell - migration -- commerce_connect_li to content_supply_chain' else null end,
    case when upsell_migration_commerce_connect_ms_to_content_supply_chain = 1 then
    ',upsell - migration -- commerce_connect_ms to content_supply_chain' else null end,
    case when upsell_migration_configured_commerce_li_to_content_supply_chain = 1 then
    ',upsell - migration -- configured_commerce_li to content_supply_chain' else null end,
    case when upsell_migration_configured_commerce_ms_to_content_supply_chain = 1 then
    ',upsell - migration -- configured_commerce_ms to content_supply_chain' else null end,
    case when upsell_migration_content_management_paas_li_to_content_supply_chain = 1 then
    ',upsell - migration -- content_management_paas_li to content_supply_chain' else null end,
    case when upsell_migration_content_management_paas_ms_to_content_supply_chain = 1 then
    ',upsell - migration -- content_management_paas_ms to content_supply_chain' else null end,
    case when upsell_migration_eol_everweb_to_content_supply_chain = 1 then
    ',upsell - migration -- eol_everweb to content_supply_chain' else null end,
    case when upsell_migration_legacy_ektron_to_content_supply_chain = 1 then
    ',upsell - migration -- legacy_ektron to content_supply_chain' else null end,
    case when upsell_migration_legacy_ektron_ms_to_content_supply_chain = 1 then
    ',upsell - migration -- legacy_ektron_ms to content_supply_chain' else null end,
    case when upsell_migration_search_navigation_to_content_supply_chain = 1 then
    ',upsell - migration -- search_navigation_standalone to content_supply_chain' else null end,
    case when upsell_migration_search_navigation_ms_to_content_supply_chain = 1 then
    ',upsell - migration -- search_navigation_standalone_ms to content_supply_chain' else null end,
    case when upsell_migration_configured_commerce_subs_to_content_supply_chain = 1 then
    ',upsell - migration -- configured_commerce_subs to content_supply_chain' else null end,
    case when upsell_migration_community_api_to_content_supply_chain = 1 then
    ',upsell - migration -- eol_community_api to content_supply_chain' else null end,

    case when upsell_migration_commerce_connect_subs_to_content_supply_chain = 1 then
    ',upsell - migration -- commerce_connect_subscriptions to content_supply_chain' else null end,
    case when upsell_migration_content_mgmt_paas_subs_to_content_supply_chain = 1 then
    ',upsell - migration -- content_management_paas_subscriptions to content_supply_chain' else null end,
    case when upsell_migration_search_nav_subs_to_content_supply_chain = 1 then
    ',upsell - migration -- search_navigation_standalone_subscriptions to content_supply_chain' else null end,
