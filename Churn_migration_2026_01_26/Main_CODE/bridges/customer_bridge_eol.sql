DROP TABLE IF EXISTS sandbox.sst_customer_bridge_eol_mig;
CREATE TABLE sandbox.sst_customer_bridge_eol_mig AS
SELECT *
FROM ufdm_archive.sst_customer_bridge_eol_temp_lcoked_20260204_1855 
...
with    base_product_group AS (
  ...
  FROM sandbox.sst_product_solution_bridge_rollup_cm_cloud
    where current_product_solution not  in ('EOL','Services' )
    or prior_product_solution not  in ('EOL','Services' )
),
...
create table sandbox.customer_churn_migration_default_cases as
    ...
    from sandbox.sst_customer_bridge_eol_mig    as a
...
Delete from sandbox.sst_customer_bridge_eol_mig as a
...
Insert into sandbox.sst_customer_bridge_eol_mig as a (
evaluation_period,
    prior_period,
    current_period,
...
FROM sandbox.sst_customer_bridge_eol_mig
GROUP BY 1,
  2,
  3,
  4,
  ...
  12,
  13;
TRUNCATE TABLE sandbox.sst_customer_bridge_eol_mig;
INSERT INTO sandbox.sst_customer_bridge_eol_mig(
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
