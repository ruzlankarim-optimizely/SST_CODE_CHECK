...
create table sandbox.churn_migration_classifiers4 as (
  ...
  initial_table_2 as (
    select *,
      ...
      case
      when current_pathways IN ('CMP') -- Added CMP
        and product_arr_change_ccfx > 0
        and prior_period_product_arr_usd_ccfx = 0
        and current_period_product_arr_usd_ccfx > 0 then 1
        else 0 end as added_cmp_in_current_date,
      case
  ...
  initial_table_3 as (
    select *,
      ...
      case
        --ai)
         -- Downgrade + CMP exists (mirror Orchestrate logic)
        when downgraded_license_in_current_date = 1
        and (sum(cmp_in_current_date_with_arr) over(partition by mcid,
            evaluation_period,
            currency_code)) > 0
        ...
        when added_cmp_in_current_date = 1 --Added a named product in the current snapshot date
        and (
          (
        ...
        when increased_cmp_in_current_date = 1 --Increased a named product in the current snapshot date
        and (
          (
            sum(downgraded_license_in_current_date) over(
              partition by mcid,
        ...
        when increased_cmp_in_current_date = 1 --Added a named product in the current snapshot date
        and (
          sum(licenses_in_previous_date_with_arr) over(
            partition by mcid,
            evaluation_period,
...
select distinct "Movement Classification" from sandbox.churn_migration_classifiers4;