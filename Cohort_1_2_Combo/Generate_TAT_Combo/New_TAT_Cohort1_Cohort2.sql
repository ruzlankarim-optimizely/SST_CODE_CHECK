...
select 
	*
from 
	ufdm.tat_upload_data tat
where 
	tat.is_deleted IS DISTINCT FROM 1
and 
    coalesce(nullif(trim(tat."Overage Y/N"), ''), 'N') is distinct from 'Y'
and not 
(
...
select 
	...
	product_family, 
	currency, 
	sum(arr) over(partition by mcid, snapshot_date, product_family, currency) as arr, 
	sum(arr_usd_ccfx) over(partition by mcid, snapshot_date, product_family, currency) as arr_usd_ccfx
from 
	edit_tat 
)
...
select  
	...
	tc2."ARR:USD CCFX"
from 
	sandbox.unified_drag_ration sdr3
inner join 
	tat_change_2 tc2 
		on 
			tc2.mcid = sdr3.mcid
		and 
			tc2.snapshot_date = sdr3.snapshot_date
--For Scenario 1 
where 
	sdr3.snapshot_date >= sdr3."Date to Drag to Under Scenario 1"::DATE
)
...
select  
	...
	tc3."NS ID", 
	tc3.subsidiary_name, 
	tc3.product_family_arr as product_family, --new product family of TAT 
	tc3.currency, 
	tc3.snapshot_date, 
	(tc3."ARR:Local Currency"*tc3."New Ratio Per Date for TAT") as arr,  -- new local currency arr of TAT 
	tc3.fx_rate_ccfx, 
	(tc3."ARR:USD CCFX"*tc3."New Ratio Per Date for TAT") as arr_usd_ccfx, --new arr_usd_ccfx of TAT 
	tc3.ccfx_date, 
	tc3.mcid_old, 
	tc3.is_deleted, 
...
from 
	edit_tat st7 
left join 
	sandbox.unified_drag_ration sdr2
		on 
			st7.mcid = sdr2.mcid
		and 
			st7.snapshot_date = sdr2.snapshot_date
--For Scenario 1 
where 
	sdr2.mcid is null 
)
...
select 
	...
	tc4."NS ID", 
	tc4.subsidiary_name, 
	tc4.product_family, --new product family of TAT 
	tc4.currency, 
	tc4.snapshot_date, 
	tc4.arr,  -- new local currency arr of TAT 
	tc4.fx_rate_ccfx, 
	tc4.arr_usd_ccfx, --new arr_usd_ccfx of TAT 
	tc4.ccfx_date, 
	tc4.mcid_old, 
	tc4.is_deleted, 
