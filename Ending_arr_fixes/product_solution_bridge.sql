CREATE OR REPLACE FUNCTION ryzlan.sp_populate_sst_product_bridge_product_solution_reversal_fix(var_period text) RETURNS void LANGUAGE plpgsql AS $function$ BEGIN
...
select distinct a.evaluation_period,
  ...
  a.current_master_customer_id,
  a.prior_master_customer_id,
  a.current_product_family as current_product_solution,
  a.prior_product_family as prior_product_solution,
...
INSERT INTO ryzlan.sst_product_bridge_product_solution_reversal_fix (
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
--     update ryzlan.sst_product_bridge_product_solution_reversal_fix a
...
temp2 as (
  ...
  from ryzlan.sst_product_bridge_product_solution_reversal_fix b
...
from ryzlan.sst_product_bridge_product_solution_reversal_fix a
  ...
  and a.evaluation_period = b.evaluation_period
  and a.current_product_solution = b.current_product_group
  left join ryzlan.sst_product_bridge_product_solution_reversal_fix c on c.mcid = a.mcid
...
delete from ryzlan.sst_product_bridge_product_solution_reversal_fix a using temp_windowngrade_final_curated b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and b.Split_record = 1
...
from ryzlan.sst_product_bridge_product_solution_reversal_fix a
  ...
  left join (
    ...
    from ryzlan.sst_product_bridge_product_solution_reversal_fix suba
...
from ryzlan.sst_product_bridge_product_solution_reversal_fix a
  ...
  join ryzlan.sst_product_bridge_product_solution_reversal_fix c on c.mcid = b.mcid
...
where b.Split_record = 1
  ...
  and a.product_bridge in ('Downgrade', 'Churn', 'Downsell')
order by mcid;
delete from ryzlan.sst_product_bridge_product_solution_reversal_fix a using temp_CPI_Reversal b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and coalesce(
...
where b.Split_record = 1
  ...
  and a.product_bridge in ('Downgrade', 'Downsell', 'Churn')
order by mcid;
delete from ryzlan.sst_product_bridge_product_solution_reversal_fix a using temp_upselldowngrade_final b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and a.current_product_solution = b.product_group_downgrade
...
--Price Ramps
--#############################################
RAISE NOTICE 'Running Price Ramps on sst product bridge solution...';
...
select pr.evaluation_period,
  ...
  cte.snapshot_date,
  pr.current_product_solution
from ryzlan.sst_product_bridge_product_solution_reversal_fix pr
  inner join cte on pr.mcid = cte.mcid
  and pr.evaluation_period = cte.evaluation_period
  and pr.current_product_solution = cte.product_solution
...
where coalesce(a.product_arr_change_ccfx::numeric, 0) > coalesce(b.PriceRamp_Value::numeric, 0)
  and a.evaluation_period = var_period
order by mcid;
delete from ryzlan.sst_product_bridge_product_solution_reversal_fix a using temp_product_bridge_price_ramps b
where 1 = 1
  and a.mcid = b.mcid
  and a.evaluation_period = b.evaluation_period
  and coalesce(a.product_arr_change_ccfx::numeric, 0) > coalesce(b.PriceRamp_Value::numeric, 0)
