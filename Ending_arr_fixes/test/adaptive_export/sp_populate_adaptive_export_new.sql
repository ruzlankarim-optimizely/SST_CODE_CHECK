CREATE OR REPLACE FUNCTION sandbox_pd.sp_populate_adaptive_exports_new(snapshot_date_from date, snapshot_date_to date) RETURNS void LANGUAGE plpgsql AS $function$ BEGIN --####################################################
...
from temp1 a
  ...
  and a.updated_product_group = d.updated_product_group --order by a.baseline_arr_local_currency
;
drop table if exists sandbox_pd.SST_adaptive_ending_ARR_split;
create table sandbox_pd.SST_adaptive_ending_ARR_split as
select snapshot_date,
  master_customer_id as c_name,
  c_full_name,
  end_customer,
