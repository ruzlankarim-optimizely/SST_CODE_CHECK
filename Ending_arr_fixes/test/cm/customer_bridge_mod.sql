DROP TABLE IF EXISTS sandbox.sst_customer_bridge_cm_ending_arr_fix;
CREATE TABLE sandbox.sst_customer_bridge_cm_ending_arr_fix AS
SELECT *,
  NULL AS pathways
FROM ryzlan.sst_customer_bridge_reversal_fix scb;
...
CREATE temp table churn_migration_classifiers_pg as (
  WITH initial_table_4 as (
    ...
    FROM sandbox.churn_migration_classifiers_ending_arr it3
      left join sandbox.sst_customer_bridge_cm_ending_arr_fix rt -- replace with product group bridge
      -- Here 
...
SELECT a.*,
  ...
  pg_bridge,
  "PG Migration: Classification"
FROM sandbox.sst_customer_bridge_cm_ending_arr_fix AS a
...
--
DELETE FROM sandbox.sst_customer_bridge_cm_ending_arr_fix AS a USING sandbox.PG_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
...
INSERT INTO sandbox.sst_customer_bridge_cm_ending_arr_fix AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
...
--
--
DELETE FROM sandbox.sst_customer_bridge_cm_ending_arr_fix AS a USING sandbox.PG_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.baseline_currency = b.baseline_currency
  AND a.customer_bridge = b.customer_bridge;
...
--
--
UPDATE sandbox.sst_customer_bridge_cm_ending_arr_fix AS a
...
FROM sandbox.sst_customer_bridge_cm_ending_arr_fix
GROUP BY 1,
  2,
  3,
  4,
  ...
  12,
  13;
TRUNCATE TABLE sandbox.sst_customer_bridge_cm_ending_arr_fix;
INSERT INTO sandbox.sst_customer_bridge_cm_ending_arr_fix(
    evaluation_period,
    prior_period,
    current_period,
    current_master_customer_id,
