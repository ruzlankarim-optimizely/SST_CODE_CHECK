...
-- Date: May 1, 2024
-- Time: 1:39:24 AM
DROP TABLE IF EXISTS sandbox.sst_product_solution_bridge_ending_arr_fix;
CREATE TABLE sandbox.sst_product_solution_bridge_ending_arr_fix AS
SELECT *,
  NULL AS pathways
FROM ryzlan.sst_product_bridge_product_solution_reversal_fix;

-- TRUNCATE TABLE sandbox.sst_product_solution_bridge_ending_arr_fix;
--#############################################
--CHURN MIGRATION
--#############################################
...
CREATE temp table churn_migration_classifiers_pg as (
  WITH initial_table_4 as (
    ...
    FROM sandbox.churn_migration_classifiers_ending_arr it3
      left join sandbox.sst_product_solution_bridge_ending_arr_fix rt -- replace with product group bridge
      -- Here 
...
SELECT a.*,
  ...
  pg_bridge,
  "PG Migration: Classification"
FROM sandbox.sst_product_solution_bridge_ending_arr_fix AS a
...
--
DELETE FROM sandbox.sst_product_solution_bridge_ending_arr_fix AS a USING sandbox.PG_migration_default AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND COALESCE (
...
INSERT INTO sandbox.sst_product_solution_bridge_ending_arr_fix AS a (
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
...
--
--
DELETE FROM sandbox.sst_product_solution_bridge_ending_arr_fix AS a USING sandbox.PG_migration_split AS b
WHERE a.mcid = b.mcid
  AND a.evaluation_period = b.evaluation_period
  AND a.currency_code = b.currency_code
  AND COALESCE (
...
--
--
UPDATE sandbox.sst_product_solution_bridge_ending_arr_fix AS a
...
FROM sandbox.sst_product_solution_bridge_ending_arr_fix
GROUP BY 1,
  2,
  3,
  4,
  ...
  19;
TRUNCATE TABLE sandbox.sst_product_solution_bridge_ending_arr_fix;
INSERT INTO sandbox.sst_product_solution_bridge_ending_arr_fix(
    evaluation_period,
    prior_period,
    current_period,
    current_end_customer,
