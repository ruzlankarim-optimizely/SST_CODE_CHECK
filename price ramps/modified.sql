create or replace function sandbox_pd.sp_populate_price_ramps_for_bridges(var_period text) returns void language plpgsql as $$ BEGIN
...
create temp table ramp AS with ramp as (
  ...
  from (
      ...
      where 1 = 1 --and snapshot_date = (select current_period from ufdm_grey.periods p  where evaluation_period = '2020M06')
        ...
        and SKU not ilike '%-OVR%'
        AND sku IN ('Full Stack', 'Web') --and reference_number in('7917','16614')
      group by 1,
        2,
        3,
        4,
