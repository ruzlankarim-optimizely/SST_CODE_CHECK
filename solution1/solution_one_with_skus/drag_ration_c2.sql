...
initial_tat as (
  select distinct mcid,
    snapshot_date,
    sum(abs(arr_usd_ccfx)) over(partition by mcid, snapshot_date) as sum_mcid_date
  from ufdm.tat_upload_data tud
  where is_deleted IS DISTINCT
  FROM 1
    and "Overage Y/N" is distinct
...
ufdm_arr_0 as (
  select distinct mcid,
    ...
    sum(abs(arr_usd_ccfx)) over(
      ...
      sum(abs(arr_usd_ccfx)) over(partition by mcid, snapshot_date),
      0
    ) as "Ratio to Each PF",
    sum(abs(arr_usd_ccfx)) over(partition by mcid, snapshot_date) as sum_ufdm_arr
