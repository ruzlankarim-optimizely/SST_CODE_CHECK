...
from temp1 a
  ...
  and a.product_family = d.product_family --order by a.baseline_arr_local_currency
;
...
case
  ...
  arr_usd_ccfx,
case
    when product_family is null
    or product_family = ''
    or product_family = 'New'
    or product_family ilike '%applicable%' then 'Recurring: Cloud: Other Bookings: Other Bookings'
    else product_family
  end,
  subsidiary_entity_name,
  c_name as c_name_real -- ,arr_total as arr_usd_ccfx_sst
,
  overage_flag
...
case
  ...
  subsidiary_entity_name,
  replace(product_bridge, 'N/A', 'Flat') as "Bridge_Account",
  'Account Name Product Group Bridge - new' as "Type",
  product_arr_change_ccfx
...
case
  ...
  subsidiary_entity_name,
  replace(product_bridge, 'N/A', 'Flat') as "Bridge_Account",
  'Account Name Product Solution Bridge - new' as "Type",
  product_arr_change_ccfx
...
temp_pb as (
  ...
  from sandbox_pd.SST_adaptive_product_bridge_movements a
