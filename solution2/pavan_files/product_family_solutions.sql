CREATE OR REPLACE FUNCTION sandbox_pd.sp_populate_sst_product_bridge_product_family_solution(var_period text) RETURNS void LANGUAGE plpgsql AS $function$ BEGIN
DELETE from sandbox_pd.sst_product_bridge_product_family_solution
where evaluation_period = var_period;
drop table if exists tmp_product_family_solution_mappings;
create temp table tmp_product_family_solution_mappings as
select 'Experiment' as Product_Solution,
  'Recurring: Cloud: Intelligence Cloud: Web Experimentation and Personalization' as Product_Family
union all
select 'Experiment',
  'Full Stack'
union all
...
union all
select 'Monetize',
  'Recurring: Cloud: Commerce Cloud: B2B Commerce (incl. Headless)'
union all
select 'Monetize',
  'Recurring: Cloud: Commerce Cloud: B2C Commerce (incl. Headless)'
union all
select 'Monetize',
  'Recurring: Cloud: Intelligence Cloud: Product Recommendations (incl. E-mail)'
union all
select 'Orchestrate',
  'Recurring: Cloud: Content Cloud: Content PaaS'
union all
select 'Orchestrate',
  'Recurring: Cloud: Intelligence Cloud: Content Recommendations (incl. E-mail)'
union all
select 'Orchestrate',
  'Recurring: Intelligence Cloud: Marketing Orchestration' --need to confirm CMP
union all
select 'Other',
  'Recurring: Cloud: Other Bookings: Campaign'
union all
select 'Other',
...
union all
select 'Other',
  '- Not Applicable -' --need to confirm
;
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--SST product Bridge
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT snapshot_date,
  ...
  b.Product_Solution as product_family,
...
FROM sandbox_pd.sst_adhoc a
  left join tmp_product_family_solution_mappings b on a.product_family = b.Product_Family
WHERE 1 = 1
  AND snapshot_date = (
    SELECT prior_period
...
create temp table arr_churned_products_tmp AS with temp as (
  ...
  from arr_new_products_tmp a
    join (
      ...
      from sandbox_pd.sst_adhoc sa
        left join tmp_product_family_solution_mappings sb on sa.product_family = sb.Product_Family
      group by 1,
        2,
        3,
        4,
    ...
    and a.product_family = b.Product_Solution
...
INSERT INTO sandbox_pd.sst_product_bridge_product_family_solution (
    evaluation_period,
    prior_period,
    current_period,
...
update sandbox_pd.sst_product_bridge_product_family_solution a
...
update sandbox_pd.sst_product_bridge_product_family_solution
...
/*
 ...
 select 'Non-Recurring: Perpetual License' as product_family , 1 as attach_arr_hierarchy , 'Perpetual License' as Product_Family_short
 union all select 'Recurring: Subscription License' , 2 ,'Subscription License'
 union all select 'Recurring: Cloud: Content Cloud: Content PaaS' , 3 , 'Content PaaS'
 union all select 'Recurring: Cloud: Content Cloud: Content SaaS' , 4 , 'Content SaaS'
 union all select 'Recurring: Cloud: Commerce Cloud: B2C Commerce (incl. Headless)' , 5, 'B2C Commerce (incl. Headless)'
 union all select 'Recurring: Cloud: Commerce Cloud: B2B Commerce (incl. Headless)' , 6, 'B2B Commerce (incl. Headless)'
 union all select 'Recurring: Cloud: Developer Cloud: Full Stack' , 7, 'Full Stack'
 union all select 'Full Stack' , 7 , 'Full Stack'
 union all select 'Recurring: Cloud: Intelligence Cloud: Web Experimentation and Personalization' , 8 , 'Web Experimentation and Personalization'
 union all select 'Web' , 8, 'Web Experimentation and Personalization'
 union all select 'Recurring: Cloud: Intelligence Cloud: CDP (incl. Visitor Intelligence)' , 9 , 'CDP (incl. Visitor Intelligence)'
 union all select 'Recurring: Cloud: Other Bookings: Campaign' , 10 , 'Campaign'
 union all select 'Recurring: Cloud: Intelligence Cloud: Content Recommendations (incl. E-mail)' , 11 , 'Content Recommendations (incl. E-mail)'
 union all select 'Recurring: Cloud: Intelligence Cloud: Product Recommendations (incl. E-mail)' , 12 , 'Product Recommendations (incl. E-mail)'
 union all select 'Recurring: Cloud: Other Bookings: Other Bookings' , 13 , 'Other'
 union all select 'Recurring: Intelligence Cloud: Marketing Orchestration' , 14 , 'Welcome'
 union all select '- Not Applicable -' , 1 , 'Perpetual License';
 
 drop table if exists tmp_churn_migration_mappings;
 
 create temporary table tmp_churn_migration_mappings as
 select 'Perpetual License' as Product_Family_short, 'Content PaaS' as Product_Family_Change
 union all select 'Perpetual License' , 'B2C Commerce (incl. Headless)'
 union all select 'Perpetual License' , 'B2B Commerce (incl. Headless)'
 --union all select 'Web Experimentation and Personalization' , 'Full Stack'
 union all select 'Other' , 'Content PaaS'
 union all select 'Other' , 'B2C Commerce (incl. Headless)'
 union all select 'Other' , 'B2B Commerce (incl. Headless)';
 ...
 ,case when current_product_family = 'Perpetual License' then 'Perpetual License'
 ...
 , t1.product_bridge
 ,coalesce (t1.current_product_family, t1.prior_product_family) as product_family
 ...
 on t1.master_customer_id = t3.mcid
