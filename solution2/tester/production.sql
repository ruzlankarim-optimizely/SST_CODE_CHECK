CREATE OR REPLACE FUNCTION sandbox_pd.sp_populate_sst_updates_manual_after_sensitivity_analysis() RETURNS void LANGUAGE plpgsql AS $function$ BEGIN
delete from sandbox_pd.sst
WHERE 1 = 1
...
from (
    ...
    where fx_type = 'ccfx'
  ) b --select * from sandbox_pd.sst a, (select trans_cur,fx_rate from ufdm_grey.arr_fx_rates where fx_type = 'ccfx') b
...
set new_product = null,
  ...
  ) --select distinct a.sku,a.product_family,a.new_product,a.new_product_solution,a.new_line_of_business,a.new_product_line,a.updated_product_group,record_source from sandbox_pd.sst a
WHERE 1 = 1
...
from tmp_mcid_mappings_vamsi b --select distinct b.*,a.parent_mcid,a.end_mcid from tmp_mcid_mappings_vamsi b,sandbox_pd.sst a
