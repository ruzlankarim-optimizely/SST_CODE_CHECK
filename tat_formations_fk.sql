...
,
  ufdm_arr_1a as (
    ...
    from ufdm_archive.arr_lcoked_03102023_2022
  ),
  ufdm_arr_1b as (
    select mcid,
      snapshot_date,
  ...
  --    distinct arr_source
  --from
  --    ufdm_archive.arr_lcoked_03102023_2022
  --where
...
--Cohort 3
DROP TABLE IF EXISTS sandbox.drag_ratio_with_sku_c3;
create table sandbox.drag_ratio_with_sku_c3 as 
(
with tat_data as 
(
select 
	distinct mcid, 
	min(snapshot_date) filter(where arr_usd_ccfx > 0) over(partition by mcid) as "Date to Drag: Sol. 1", 
	date_trunc('MONTH',('2021-12-01'::DATE)) as "MAX Snapshot Date of TAT", 
	case 
		when mcid = 'baf411ad-6ca1-e811-8154-70106fa6f451' then 'f488e6f7-1faf-1c43-f6aa-378826baaec9'
		when mcid = 'Special' then '8820a932-1eaa-db11-8952-0018717a8c8'
		when mcid = 'b8fee10b-d73b-e543-989d-09b7f6c70ba4' then '4c94a000-8a00-e611-812a-c4346baccd14'
		when mcid = '18ba887d-2626-0e28-bfce-82a9148c0dfe' then 'b2d0c4ef-e8e0-4232-9e12-23df280c6bec'
		when mcid = '50ffab14-1eaa-db11-8952-0018717a8c82' then '951db62-13bb-ea11-a812-000d3abb4345'
		when mcid = 'f68b6663-fc8f-186f-fe98-a82fc3decf13' then '6357f282-60e4-e411-9afb-0050568d2da8'
	end as mcid_arr 
from 
...
where 
	mcid in 
	(
	'baf411ad-6ca1-e811-8154-70106fa6f451',
	'Special',
	'b8fee10b-d73b-e543-989d-09b7f6c70ba4',
	'18ba887d-2626-0e28-bfce-82a9148c0dfe',
	'50ffab14-1eaa-db11-8952-0018717a8c82',
	'f68b6663-fc8f-186f-fe98-a82fc3decf13'
	)
)
...
select 
	...
	sku, 
	snapshot_date, 
	sum(arr_usd_ccfx) over(partition by mcid, sku, snapshot_date)/nullif(sum(arr_usd_ccfx) over(partition by mcid, snapshot_date),0) as "Ratio of ARR"
from 
	ufdm.arr 
where 
...
select 
	ud.mcid as mcid_arr, 
	td."MAX Snapshot Date of TAT", 
	ud.product_family as product_family_arr, 
	ud.sku, 
	ud."Ratio of ARR", 
	td."Date to Drag: Sol. 1"
from
	ufdm_data ud 
inner join 
	tat_data td 
...
create table sandbox.drag_ratio_with_sku_with_refined_proposal as (
  with combined_drag_ratio as (
    ...
    UNION ALL  
    	...
    	"Date to Drag: Sol. 1"
    FROM 
    	sandbox.drag_ratio_with_sku_c3
  ),
  remove_customer AS (
    SELECT DISTINCT mcid_arr
    FROM (
...
DROP TABLE IF EXISTS sandbox.tat_with_sku_with_refined_proposal_version2;
CREATE TABLE sandbox.tat_with_sku_with_refined_proposal_version2 as (
  with edit_tat as (
    select *
