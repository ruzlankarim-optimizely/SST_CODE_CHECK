CREATE OR REPLACE FUNCTION ryzlan.sp_populate_sst_customer_bridge_quaterly(var_period text) RETURNS void LANGUAGE plpgsql AS $function$ BEGIN
DELETE from ryzlan.sst_customer_bridge_quaterly
where evaluation_period = var_period;
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--SST customer Bridge
-- CAST(date_trunc('quarter',  snapshot_date)  + interval '3 months' - interval '1 day' AS date)
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
WHERE 1 = 1
  AND snapshot_date in (
    SELECT distinct prior_quarter
    from ufdm_grey.period_quarterly
    WHERE evaluation_period = var_period
  )
...
--     select * from customer_level_arr;
...
create temp table arr_bridge_tmp as with arr_bridge AS (
  SELECT per.evaluation_period as evaluation_period,
    cla.prior_period,
    cla.current_period,
  ...
  FROM customer_level_arr cla
    ...
    and a.rn = 1
    CROSS JOIN ufdm_grey.period_quarterly per
  WHERE 1 = 1
...
--     select * from arr_new_products_tmp
...
INSERT INTO ryzlan.sst_customer_bridge_quaterly (
    evaluation_period,
    prior_period,
    current_period,
...
with base as (
  ...
  from sandbox_pd.sst a
  where a.snapshot_date in (
      SELECT prior_quarter
      from ufdm_grey.period_quarterly
      WHERE evaluation_period = var_period
      union
...
update ryzlan.sst_customer_bridge_quaterly a
...
update ryzlan.sst_customer_bridge_quaterly
